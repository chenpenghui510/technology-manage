	...
	public Result<String> sms(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			if (CommonConstant.SMS_TPL_TYPE_1.equals(smsmode)) {
				...
				String paramJson="{\"code\":\""+captcha+"\"}";
				String content="您的注册码："+captcha+"，如非本人操作，请忽略本短信！";
				b = DySmsHelper.sendSms(1,"注册",mobile, captcha,content, DySmsHelper.REGISTER_TEMPLATE_CODE,paramJson);
			}else {
				//登录模式，校验用户有效性(手机登录暂时取消)
				/*SysUser sysUser = sysUserService.getUserByPhone(mobile);
				result = sysUserService.checkUserIsEffective(sysUser);
				if(!result.isSuccess()) {
					return result;
				...
				if (CommonConstant.SMS_TPL_TYPE_0.equals(smsmode)) {
					//登录模板
					//b = DySmsHelper.sendSms(mobile, captcha,sysUser.getUsername(), DySmsHelper.LOGIN_TEMPLATE_CODE);
				} else if(CommonConstant.SMS_TPL_TYPE_2.equals(smsmode)) {
					Integer userType=Integer.valueOf(jsonObject.get("userType").toString());
					SysUser sysUser = sysUserService.getUserByPhone(mobile);
					//忘记密码模板
					String paramJson="{\"code\":\""+captcha+"\"}";
					String content="您的动态码为："+captcha+"，您正在进行密码重置操作，如非本人操作，请忽略本短信！";
					b = DySmsHelper.sendSms(1,"找回密码",mobile, captcha,content, DySmsHelper.FORGET_PASSWORD_TEMPLATE_CODE,paramJson);
				}else if(CommonConstant.SMS_TPL_TYPE_3.equals(smsmode)){
					String paramJson="{\"code\":\""+captcha+"\"}";
					String content="您的动态码为："+captcha+"，您正在查询专家完善信息，如非本人操作，请忽略本短信！";
					b = DySmsHelper.sendSms(1,"查询专家完善信息",mobile, captcha,content, DySmsHelper.FORGET_PASSWORD_TEMPLATE_CODE,paramJson);
				}
			}
