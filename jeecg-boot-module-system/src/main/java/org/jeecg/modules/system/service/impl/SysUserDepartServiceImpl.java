...
public class SysUserDepartServiceImpl extends ServiceImpl<SysUserDepartMapper, SysUserDepart> implements ISysUserDepartService {
	...
	public boolean editSysUserWithDepart(SysUserDepartsVO sysUserDepartsVO) {
		...
		if(depIdList != null && depIdList.size() > 0) {
			...
			List<SysUserDepart> uDepList = this.list(queryDep);
			if(uDepList.size()!=0){
				if(!uDepList.get(0).getDepId().equals(sysUserDepartsVO.getDepartIdList().get(0))){
					List<UserBusiness> list1 = userBusinessMapper.getEntityBy(uDepList.get(0).getUserId(), null);
					if(list1!=null&&list1.size()>0){
						return ok;
					}
				}
				ok= this.remove(queryDep);
				if(ok=false){
					return ok;
				}
			}
